import asyncio

from prefect import flow

from src.modules.database import init_db
from src.tasks.fetch_kalshi_data import fetch_kalshi_data


@flow(name="fetch-kalshi-data")
def fetch_kalshi_data_flow():
    """Flow to fetch data from Kalshi."""
    print("Starting data fetch from Kalshi...")
    
    # Initialize database tables if they don't exist
    # print("Initializing database tables...")
    # try:
    #     asyncio.run(init_db())
    #     print("Database tables initialized successfully!")
    # except Exception as e:
    #     print(f"Warning: Database initialization failed: {e}")
    #     print("Continuing anyway - tables may already exist...")
    
    # Fetch data from Kalshi
    kalshi_result = fetch_kalshi_data()
    
    print("\nCompleted fetching data from Kalshi!")
    return kalshi_result


if __name__ == "__main__":
    fetch_kalshi_data_flow()

